...
<script>
    ...
    $(document).ready(function () {
        ...
        $('.btnBuy').click(function () {
            ...
            $('.popup2').show();
            makeCode();
            $('#payAmt').html(($('#popupPrice').html().replace(" ADA","") * $('#popupBuyNum').val()) + " ADA");
            $('#payAmtIn').val(($('#popupPrice').html().replace(" ADA","") * $('#popupBuyNum').val()));
            return false;
        });
        //X 누르면 팝업닫기
    ...
    function collectClick(nftId){
        ...
        $.ajax({
            ...
            , success:function(data) {
                ...
                $('#payAddr').html(data.response.address);
    ...
    function checkSoldOut(){
        ...
        $.ajax({
            ...
            , success:function(data) {
                ...
                $('#popupAge').html(data.response.age);
                $('#payAddr').html(data.response.address);
                $('#payAddrIn').val(data.response.address);
                $('#popupDescription').html(data.response.description);
                $('#popupPrice').html((data.response.nftPrice / 1000000) + " ADA");
            }
            , error:function(request,status,error) {
        ...
        <div class="popupWrap">
            ...
            <div class="popup2">
                ...
                <div class="popupText">
                    ...
                    <div class="popupBox"><span id="payAddr"></span><input type="hidden" id="payAddrIn">
                        <button class="btnCopy" onclick="copyText('payAddrIn');"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M22 6v16h-16v-16h16zm2-2h-20v20h20v-20zm-24 17v-21h21v2h-19v19h-2z"></path></svg></button>
                    </div>
                    <div class="popupSubtitle">Amount</div>
                    <div class="popupBox"><span id="payAmt">2 ADA</span><input type="hidden" id="payAmtIn">
                        <button class="btnCopy" onclick="copyText('payAmtIn');"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M22 6v16h-16v-16h16zm2-2h-20v20h20v-20zm-24 17v-21h21v2h-19v19h-2z"></path></svg></button>
                    </div>
